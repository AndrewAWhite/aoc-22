package main

import (
	"fmt"

	"github.com/AndrewAWhite/aoc-22/solutions"
)

func complete() {
	fmt.Printf("Day 1, Part 1: %d\n", solutions.Solution_01_1())
	fmt.Printf("Day 1, Part 1: %d\n", solutions.Solution_01_2())

	fmt.Printf("Day 2, Part 1: %d\n", solutions.Solution_02_1())
	fmt.Printf("Day 2, Part 2: %d\n", solutions.Solution_02_2())

	fmt.Printf("Day 3, Part 1: %d\n", solutions.Solution_03_1())
	fmt.Printf("Day 3, Part 2: %d\n", solutions.Solution_03_2())

	fmt.Printf("Day 4, Part 1: %d\n", solutions.Solution_04_1())
	fmt.Printf("Day 4, Part 2: %d\n", solutions.Solution_04_2())

	fmt.Printf("Day 5, Part 1: %s\n", solutions.Solution_05_1())
	fmt.Printf("Day 5, Part 2: %s\n", solutions.Solution_05_2())

	fmt.Printf("Day 6, Part 1: %d\n", solutions.Solution_06_1())
	fmt.Printf("Day 6, Part 2: %d\n", solutions.Solution_06_2())

	fmt.Printf("Day 7, Part 1: %d\n", solutions.Solution_07_1())
	fmt.Printf("Day 7, Part 2: %d\n", solutions.Solution_07_2())

	fmt.Printf("Day 8, Part 1: %d\n", solutions.Solution_08_1())
	fmt.Printf("Day 8, Part 2: %d\n", solutions.Solution_08_2())

	fmt.Printf("Day 9, Part 1: %d\n", solutions.Solution_09_1())
	fmt.Printf("Day 9, Part 2: %d\n", solutions.Solution_09_2())

	fmt.Printf("Day 10, Part 1: %d\n", solutions.Solution_10_1())
	fmt.Printf("Day 10, Part 2:\n%s\n", solutions.Solution_10_2())

	fmt.Printf("Day 11, Part 1: %d\n", solutions.Solution_11_1())
	fmt.Printf("Day 11, Part 2: %d\n", solutions.Solution_11_2())

	fmt.Printf("Day 12, Part 1: %d\n", solutions.Solution_12_1())
	fmt.Printf("Day 12, Part 2: %d\n", solutions.Solution_12_2())

	fmt.Printf("Day 13, Part 1: %d\n", solutions.Solution_13_1())
	fmt.Printf("Day 13, Part 2: %d\n", solutions.Solution_13_2())

	fmt.Printf("Day 14, Part 1: %d\n", solutions.Solution_14_1())
	fmt.Printf("Day 14, Part 2: %d\n", solutions.Solution_14_2())

	fmt.Printf("Day 15, Part 1: %d\n", solutions.Solution_15_1())
	fmt.Printf("Day 15, Part 2: %d\n", solutions.Solution_15_2())
}
